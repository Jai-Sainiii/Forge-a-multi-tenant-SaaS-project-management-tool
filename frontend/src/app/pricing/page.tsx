...
const PricingCard = ({ title, price, features, highlighted = false, onAction }: { title: string, price: string, features: string[], highlighted?: boolean, onAction: () => void }) => (
  <div className={`p-10 bg-white dark:bg-slate-900 rounded-xl flex flex-col gap-8 relative border-2 ${highlighted ? 'border-primary shadow-xl shadow-primary/5' : 'border-outline-variant dark:border-slate-800'}`}>
    ...
    <button
      onClick={onAction}
      className={`w-full py-3.5 rounded-lg font-medium transition-all active:scale-95 border cursor-pointer ${highlighted ? 'bg-primary text-white border-primary hover:bg-primary-light' : 'bg-transparent text-slate-800 dark:text-slate-200 border-outline-variant dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
    >
...
export default function Pricing() {
    ...
    const handleContactSales = () => {
      window.location.href = "mailto:sales@forge.pm?subject=Enterprise%20Inquiry";
    };
      ...
      <>
        <section id="pricing" className="max-w-7xl mx-auto px-6 py-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <PricingCard title="Starter" price="$0" features={['Up to 3 members', '2 projects', 'Community support']} onAction={handleStartFree} />
            <PricingCard title="Pro" price="$19" features={['Unlimited members', 'Unlimited projects', 'Advanced analytics', 'Priority support']} highlighted={true} onAction={handleStartFree} />
            <PricingCard title="Enterprise" price="Custom" features={['SSO & Audit logs', 'Dedicated success manager', 'Custom integrations']} onAction={handleContactSales} />
          </div>
        </section>
