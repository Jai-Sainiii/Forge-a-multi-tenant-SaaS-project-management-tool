...
import { getTaskDetail, submitTask, updateTask } from "@/app/actions/task";
...
function EditTaskModal({
  ...
  const handleUpdateTask = async () => {
    ...
    setEditError(null);
    try {
      const result = await updateTask(String(task.id), {
        title: editTitle,
      ...
      if (result.success) {
        // reload the details to get the populated project name and assignees correctly
        const reloadRes = await getTaskDetail(String(task.id));
        if (reloadRes.success && reloadRes.task) {
          onSuccess(reloadRes.task);
        } else {
          onClose();
        }
        onClose();
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[
                ...
                { value: "medium", label: "Medium", icon: Minus, color: "text-gray-500" },
          ...
          <div>
            ...
            {projectMembers.length > 0 ? (
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">No members available in this project.</p>
            )}
          </div>
        </div>
...
export default function TaskDetailClient({
  ...
  const [projectMembers, setProjectMembers] = useState<any[]>(initialProjectMembers);
  ...
  useEffect(() => {
    if (task && task.projectId && initialProjectMembers.length === 0) {
      const fetchProjectMembers = async () => {
        try {
          const res = await getSingleProject(String(task.projectId));
