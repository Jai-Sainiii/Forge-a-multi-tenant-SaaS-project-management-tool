...
import { Sparkles, FolderKanban, Plus, CheckSquare, Loader2, Play, ChevronRight, Activity, Calendar } from "lucide-react";
...
export default function WorkspaceDashboard({ params }: { params: Promise<{ workspaceID: string }> }) {
  ...
  // Click handler to toggle task checked state in local and DB
  const handleToggleTaskStatus = async (taskId: number, currentStatus: string) => {
    ...
    try {
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/tasks/status/${taskId}`,
        { status: newStatus },
        { withCredentials: true }
      );
      if (!res.data.success) {
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "80vh", gap: 16 }}>
        <Loader2 size={36} className="animate-spin text-[#6C5CE7]" />
        <p style={{ fontSize: 14, color: "#9A9890", fontWeight: 500 }}>Loading Dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1100 }} className="p-4 sm:p-6 md:p-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {/* Render reusable CreateTaskModal on dashboard */}
      {isCreateTaskOpen && (
        <CreateTaskModal
